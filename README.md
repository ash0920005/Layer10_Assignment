...
This system was developed for the Layer10 Take-Home Project, focusing on the challenge of maintaining "correctness over time" in a world where messages are edited, decisions are reversed, and data is fragmented across platforms.
🚀 Key Features
Semantic Entity Resolution: Uses Vector Embeddings (all-minilm) to merge entities like "momo app" and "com.immomo.momo" based on mathematical meaning, not just text matching.
Multi-Source Fusion: Automatically connects technical artifacts (GitHub) with human discussions (Slack) to provide a unified organizational "Brain."
...
Grounded Retrieval: Every fact in the memory graph is strictly traced to an evidence_url (GitHub Link or Slack ID) and a source_type.
High-Visibility Visualization: An interactive, spacious graph UI with a color-coded legend for easy exploration of the organization's memory.
🛠️ Tech Stack
...
Data Storage	Standardized JSON Knowledge Store
📂 Project Structure
code
Text
...
🚦 Getting Started
1. Prerequisites
Install Ollama and download the required open-weight models:
code
Bash
ollama pull llama3
...
Run the scripts in order to build the memory from scratch:
code
Bash
...
Standard systems fail when names collide or change. We implemented Semantic Deduplication. Using a similarity threshold of 0.82, the system calculates the cosine similarity of entity vectors.
...
The extraction script utilizes a processed_log.txt. This ensures Idempotency—the pipeline can be run incrementally. It will only process new messages, making it viable for long-term production use.
...
📂 TECHNICAL ARTIFACTS (GitHub Issues):
   • [2023-01-01] com.immomo.momo has_issue 59c0de8f...
